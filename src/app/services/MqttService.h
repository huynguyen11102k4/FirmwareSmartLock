...
public:
  ...
  void attachCallback();
  void onConnected(int infoVersion);

private:
  static void callbackThunk(char* topic, byte* payload, unsigned int length);
  void dispatch_(const String& topicStr, const String& payloadStr);

  void handlePasscodesTopic_(const String& payloadStr);
  void handleIccardsTopic_(const String& payloadStr);
  void handleControlTopic_(const String& payloadStr);
