...
void HttpService::loop() { server_.handleClient(); }

void HttpService::handleInfo_() {
  const int batt = readBatteryPercent_ ? readBatteryPercent_(ctx_) : 0;

  DynamicJsonDocument doc(256);
  doc["mac"] = appState_.deviceMAC;
  doc["topic"] = appState_.baseTopic;
  doc["battery"] = batt;
...
void HttpService::handleNotFound_() { server_.send(404, "text/plain", "Not found"); }
