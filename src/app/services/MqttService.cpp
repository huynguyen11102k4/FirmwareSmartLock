...
MqttService::MqttService(AppState& appState,
                         ...
                         void* ctx,
                         SyncFn syncIccardsCache,
                         BatteryFn readBatteryPercent,
                         UnlockFn onUnlock,
                         LockFn onLock)
  : appState_(appState),
    passRepo_(passRepo),
    ...
    publish_(publish),
    ctx_(ctx),
    syncIccardsCache_(syncIccardsCache),
    readBatteryPercent_(readBatteryPercent),
    onUnlock_(onUnlock),
    onLock_(onLock) {}

void MqttService::attachCallback() {
  s_instance_ = this;
  MqttManager::setCallback(&MqttService::callbackThunk);
}

void MqttService::onConnected(int infoVersion) {
  const String base = appState_.baseTopic;
...
void MqttService::dispatch_(const String& topicStr, const String& payloadStr) {
  ...
  if (topicStr == Topics::passcodes(base)) return handlePasscodesTopic_(payloadStr);
  if (topicStr == Topics::passcodesReq(base)) return publish_.publishPasscodeList();
  if (topicStr == Topics::iccards(base)) return handleIccardsTopic_(payloadStr);
  if (topicStr == Topics::iccardsReq(base)) return publish_.publishICCardList();
  if (topicStr == Topics::control(base)) return handleControlTopic_(payloadStr);
}

void MqttService::handlePasscodesTopic_(const String& payloadStr) {
  DynamicJsonDocument doc(512);
  if (!JsonUtils::deserialize(payloadStr, doc)) return;
  ...
  if (action == "add" && type == "permanent") {
    ...
    if (!hasMaster) {
      if (newCode.length() < 4 || newCode.length() > 10) return;
    ...
    if (oldCode.isEmpty() || oldCode != current) {
      MqttManager::publish(Topics::passcodesError(appState_.baseTopic),
...
void MqttService::handleIccardsTopic_(const String& payloadStr) {
  ...
  if (action == "add" && id.length() > 0) {
    ...
    if (cardRepo_.add(uid)) {
      if (syncIccardsCache_) syncIccardsCache_(ctx_);
      publish_.publishICCardList();
  ...
  if (action == "start_swipe_add") {
    ...
    appState_.flags.swipeAddMode = true;
    MqttManager::publish(Topics::iccardsStatus(appState_.baseTopic), "swipe_add_started");
    return;
  }
}
...
void MqttService::handleControlTopic_(const String& payloadStr) {
  ...
  const String action = doc["action"] | "";
  if (action == "unlock") {
    if (onUnlock_) onUnlock_(ctx_, "remote");
  } else if (action == "lock") {
    if (onLock_) onLock_(ctx_, "remote");
  }
}