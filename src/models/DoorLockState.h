...
struct DoorLockState
{
    ...
    void unlock(uint32_t durationMs = DEFAULT_UNLOCK_DURATION_MS)
    {
    ...
    uint32_t remainingUnlockSeconds() const
    {
        if (state != State::UNLOCKED) return 0;
        if (millis() >= relockAtMs) return 0;
